...
import{j as e}from"./jsx-runtime.D_zvdyIk.js";import{r as y}from"./index.DiEladB3.js";import{_ as re}from"./preload-helper.BlTxHScW.js";import{r as Ce}from"./index.Pl_XcmgS.js";function ve(o){const d=document.body;for(let h=o.parentElement;h&&h!==d;h=h.parentElement){const x=getComputedStyle(h).overflowY;if(x==="auto"||x==="scroll")return h}return null}const He=.06,ye=.0085,Be=.006,Z=1.45,De=8;function Re({modelUrl:o,poster:d,posterAlt:h,accent:x,yaw:H=0,exposure:D=1,orient:P}){const a=y.useRef(null),[p,A]=y.useState(!1),[u,b]=y.useState("poster"),[j,N]=y.useState(!1);y.useEffect(()=>{if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const n=a.current;if(!n)return;let L=!1;const s=()=>{L||A(!0)},t=()=>{const l=window;l.requestIdleCallback?l.requestIdleCallback(s,{timeout:1200}):window.setTimeout(s,200)},i=()=>{const l=(document.getAnimations?.()??[]).filter(c=>c.playState==="running"&&/^cs-(main|page)-in$/.test(c.animationName??""));l.length?Promise.allSettled(l.map(c=>c.finished)).then(t,t):t()},g=new IntersectionObserver(([l])=>{l.isIntersecting&&(g.disconnect(),i())},{root:ve(n),rootMargin:"400px 0px"});return g.observe(n),()=>{L=!0,g.disconnect()}},[]),y.useEffect(()=>{if(!p)return;const n=a.current;if(!n)return;let L=!1,s=null;return(async()=>{const[t,{GLTFLoader:i},{RoomEnvironment:g}]=await Promise.all([re(()=>import("./three.module.Bx43vjkH.js"),[]),re(()=>import("./GLTFLoader.BqdKpcjx.js"),__vite__mapDeps([0,1])),re(()=>import("./RoomEnvironment.Ct7rVCVU.js"),__vite__mapDeps([2,1]))]);if(L)return;let l;try{if(l=new t.WebGLRenderer({antialias:!0,alpha:!0}),!l.getContext())throw new Error("no webgl context")}catch(f){console.warn("CaseStudyStage: WebGL unavailable, keeping poster",f),b("failed");return}l.setPixelRatio(Math.min(window.devicePixelRatio,2)),l.toneMapping=t.ACESFilmicToneMapping,l.toneMappingExposure=.82*D,l.outputColorSpace=t.SRGBColorSpace,l.setSize(n.clientWidth,n.clientHeight),n.appendChild(l.domElement);const c=l.domElement;c.style.width="100%",c.style.height="100%",c.style.display="block",c.style.touchAction="pan-y";const T=new t.Scene,E=new t.PerspectiveCamera(38,n.clientWidth/n.clientHeight,.1,100);E.position.set(0,0,5.4);const C=new t.PMREMGenerator(l),S=C.fromScene(new g,.04).texture;T.environment=S,T.environmentIntensity=.55;const w=new t.Color(x),z=w.clone().lerp(new t.Color("#ffffff"),.55),B=new t.DirectionalLight(16777215,.5);B.position.set(0,3,5);const W=new t.DirectionalLight(w,1);W.position.set(-5,1.5,-3.5);const k=new t.DirectionalLight(z,.7);k.position.set(4.5,2,3),T.add(B,W,k);const m=new t.Group,M=new t.Group;M.rotation.y=H,m.add(M),T.add(m);const F=(f,Y)=>{const G=new t.Box3().setFromObject(f),O=G.getSize(new t.Vector3),U=G.getCenter(new t.Vector3),Pe=Math.max(O.x,O.y,O.z)||1,we=Y/Pe;f.scale.setScalar(we),f.position.copy(U).multiplyScalar(-we)};new i().load(o,f=>{if(L)return;const Y=f.scene,G=new t.Group;P&&G.rotation.set(P[0],P[1],P[2]),G.add(Y),F(G,2.8);const O=()=>{L||(M.add(G),b("ready"))};l.compileAsync(G,E,T).then(O,O)},void 0,f=>{console.warn("CaseStudyStage: GLB failed to load, keeping poster",f),b("failed")});let R=!1,I=!1,q=0,_=0,se=0,ie=0,$=0;const Ee=f=>f<-Z?-Z:f>Z?Z:f,ne=f=>{f.pointerType==="mouse"&&f.button!==0||(R=!0,I=f.pointerType!=="touch",q=se=f.clientX,_=ie=f.clientY,$=0,I&&c.setPointerCapture?.(f.pointerId),N(!0))},le=f=>{if(!R)return;if(!I){const O=f.clientX-se,U=f.clientY-ie;if(Math.hypot(O,U)<De)return;if(Math.abs(O)<=Math.abs(U)){R=!1,N(!1);return}I=!0,c.setPointerCapture?.(f.pointerId)}const Y=f.clientX-q,G=f.clientY-_;q=f.clientX,_=f.clientY,M.rotation.y+=Y*ye,m.rotation.x=Ee(m.rotation.x+G*Be),$=$*.6+Y*ye*.4},K=()=>{R&&(R=!1,I=!1,N(!1))};c.addEventListener("pointerdown",ne),c.addEventListener("pointermove",le),c.addEventListener("pointerup",K),c.addEventListener("pointercancel",K);let V=!1;const ce=f=>{f.pointerType==="mouse"&&(V=!0)},de=()=>{V=!1};n.addEventListener("pointerenter",ce),n.addEventListener("pointerleave",de);const he=new t.Clock;let Q=0,ee=!1;const Le=()=>{const f=Math.min(he.getDelta(),.05);if(!R){const Y=V?0:He*f;$+=(Y-$)*(V?.09:.03),V&&Math.abs($)<1e-5&&($=0),M.rotation.y+=$}l.render(T,E)},pe=()=>{Le(),Q=requestAnimationFrame(pe)},ze=()=>{ee||(ee=!0,he.start(),Q=requestAnimationFrame(pe))},me=()=>{ee=!1,cancelAnimationFrame(Q)};let ge=!0;const te=()=>{ge&&!document.hidden?ze():me()},ue=new IntersectionObserver(([f])=>{ge=f.isIntersecting,te()},{root:ve(n),threshold:0});ue.observe(n);const fe=()=>te();document.addEventListener("visibilitychange",fe);const ae=()=>{!n.clientWidth||!n.clientHeight||(E.aspect=n.clientWidth/n.clientHeight,E.updateProjectionMatrix(),l.setSize(n.clientWidth,n.clientHeight))};window.addEventListener("resize",ae);const be=new ResizeObserver(ae);be.observe(n),te(),s=()=>{me(),ue.disconnect(),be.disconnect(),document.removeEventListener("visibilitychange",fe),window.removeEventListener("resize",ae),c.removeEventListener("pointerdown",ne),c.removeEventListener("pointermove",le),c.removeEventListener("pointerup",K),c.removeEventListener("pointercancel",K),n.removeEventListener("pointerenter",ce),n.removeEventListener("pointerleave",de),S.dispose(),C.dispose(),l.dispose(),c.parentNode===n&&n.removeChild(c)}})(),()=>{L=!0,s?.()}},[p,o,x,H,D,P]);const v=u!=="ready";return e.jsxs("div",{className:"cs-stage",style:{cursor:u==="ready"?j?"grabbing":"grab":"default"},children:[e.jsx("div",{ref:a,className:"cs-stage-mount","aria-hidden":u==="ready"?void 0:!0}),e.jsx("img",{src:d,alt:h,draggable:!1,className:"cs-stage-poster",style:{opacity:v?1:0},loading:"lazy"}),u==="ready"&&e.jsxs("span",{className:"cs-stage-hint",style:{color:x},children:[e.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round","aria-hidden":"true",children:e.jsx("path",{d:"M12 4v16M4 12h16M12 4l-3 3M12 4l3 3M12 20l-3-3M12 20l3-3M4 12l3-3M4 12l3 3M20 12l-3-3M20 12l-3 3"})}),"Drag to rotate"]}),e.jsx("style",{children:`
        .cs-stage {
          position: relative;
          width: 100%;
          height: 100%;
      ...
      `})]})}const Ie=.26,Ge=.2,ke=30,xe=17,je=.93,Oe=.045,Te=3;function Fe({hero:o,heroAlt:d,backdrop:h,accent:x}){const H=y.useRef(null),D=y.useRef(null),[P,a]=y.useState(!1);return y.useEffect(()=>{const p=H.current,A=D.current;if(!p||!A)return;const u=window.matchMedia("(prefers-reduced-motion: reduce)").matches;let b=0,j=0,N=0,v=0,n=!1,L=!1,s=0,t=0,i=0;const g=(w,z)=>w<-z?-z:w>z?z:w,l=()=>{A.style.transform=`perspective(1100px) rotateX(${j.toFixed(3)}deg) rotateY(${b.toFixed(3)}deg)`},c=()=>{if(i=0,n)return;if(Math.abs(N)>.05||Math.abs(v)>.05)b=g(b+N,ke),j=g(j+v,xe),N*=je,v*=je;else{N=0,v=0;const z=u?.25:Oe;if(b+=(0-b)*z,j+=(0-j)*z,Math.abs(b)<.04&&Math.abs(j)<.04){b=0,j=0,l();return}}l(),i=requestAnimationFrame(c)},T=()=>{!i&&!n&&(i=requestAnimationFrame(c))},E=w=>{w.pointerType==="mouse"&&w.button!==0||(L=!0,n=!1,s=w.clientX,t=w.clientY,N=0,v=0,i&&(cancelAnimationFrame(i),i=0))},C=w=>{if(!L)return;const z=w.clientX-s,B=w.clientY-t;if(!n){if(Math.abs(z)<Te&&Math.abs(B)<Te)return;if(w.pointerType!=="mouse"&&Math.abs(B)>Math.abs(z)){L=!1;return}n=!0,a(!0),p.classList.add("csis-dragging"),p.setPointerCapture?.(w.pointerId)}s=w.clientX,t=w.clientY,N=z*Ie,v=B*Ge,b=g(b+N,ke),j=g(j+v,xe),l()},S=w=>{!L&&!n||(L=!1,n&&(n=!1,p.classList.remove("csis-dragging"),p.releasePointerCapture?.(w.pointerId),u&&(N=0,v=0)),T())};return p.addEventListener("pointerdown",E),p.addEventListener("pointermove",C),p.addEventListener("pointerup",S),p.addEventListener("pointercancel",S),()=>{p.removeEventListener("pointerdown",E),p.removeEventListener("pointermove",C),p.removeEventListener("pointerup",S),p.removeEventListener("pointercancel",S),i&&cancelAnimationFrame(i)}},[]),e.jsxs("div",{ref:H,className:"csis",children:[e.jsx("img",{src:h,alt:"","aria-hidden":"true",className:"csis-backdrop",draggable:!1}),e.jsx("div",{className:"csis-scrim","aria-hidden":"true"}),e.jsx("div",{ref:D,className:"csis-tilt",children:e.jsx("img",{src:o,alt:d,className:"csis-hero",draggable:!1})}),!P&&e.jsxs("span",{className:"csis-hint",style:{color:x},children:[e.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round","aria-hidden":"true",children:e.jsx("path",{d:"M4 12h16M4 12l4-4M4 12l4 4M20 12l-4-4M20 12l-4 4"})}),"Drag to turn"]}),e.jsx("style",{children:`
        .csis {
          position: relative;
          width: 100%;
          height: 100%;
