...
import{j as e}from"./jsx-runtime.D_zvdyIk.js";import{r as j}from"./index.DiEladB3.js";import{_ as Y}from"./preload-helper.BlTxHScW.js";const ge=.25,se=.0085;function me({modelUrl:i,poster:s,posterAlt:c,accent:y,yaw:H=0}){const a=j.useRef(null),[b,u]=j.useState(!1),[S,T]=j.useState("poster"),[f,w]=j.useState(!1);j.useEffect(()=>{if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const r=a.current;if(!r)return;const p=new IntersectionObserver(([d])=>{d.isIntersecting&&(u(!0),p.disconnect())},{rootMargin:"400px 0px"});return p.observe(r),()=>p.disconnect()},[]),j.useEffect(()=>{if(!b)return;const r=a.current;if(!r)return;let p=!1,d=null;return(async()=>{const[o,{GLTFLoader:v},{RoomEnvironment:E}]=await Promise.all([Y(()=>import("./three.module.Bx43vjkH.js"),[]),Y(()=>import("./GLTFLoader.BqdKpcjx.js"),__vite__mapDeps([0,1])),Y(()=>import("./RoomEnvironment.Ct7rVCVU.js"),__vite__mapDeps([2,1]))]);if(p)return;let h;try{if(h=new o.WebGLRenderer({antialias:!0,alpha:!0}),!h.getContext())throw new Error("no webgl context")}catch(m){console.warn("CaseStudyStage: WebGL unavailable, keeping poster",m),T("failed");return}h.setPixelRatio(Math.min(window.devicePixelRatio,2)),h.toneMapping=o.ACESFilmicToneMapping,h.toneMappingExposure=1.05,h.outputColorSpace=o.SRGBColorSpace,h.setSize(r.clientWidth,r.clientHeight),r.appendChild(h.domElement);const g=h.domElement;g.style.width="100%",g.style.height="100%",g.style.display="block",g.style.touchAction="pan-y";const k=new o.Scene,x=new o.PerspectiveCamera(38,r.clientWidth/r.clientHeight,.1,100);x.position.set(0,0,5.4);const A=new o.PMREMGenerator(h),B=A.fromScene(new E,.04).texture;k.environment=B;const R=new o.Color(y),I=R.clone().lerp(new o.Color("#ffffff"),.55),n=new o.DirectionalLight(16777215,1.4);n.position.set(0,3,5);const l=new o.DirectionalLight(R,3);l.position.set(-5,1.5,-3.5);const L=new o.DirectionalLight(I,2.2);L.position.set(4.5,2,3),k.add(n,l,L);const M=new o.Group;M.rotation.y=H,k.add(M);const G=(m,z)=>{const ie=new o.Box3().setFromObject(m),U=ie.getSize(new o.Vector3),pe=ie.getCenter(new o.Vector3),he=Math.max(U.x,U.y,U.z)||1,oe=z/he;m.scale.setScalar(oe),m.position.copy(pe).multiplyScalar(-oe)};new v().load(i,m=>{if(p)return;const z=m.scene;G(z,2.8),M.add(z),T("ready")},void 0,m=>{console.warn("CaseStudyStage: GLB failed to load, keeping poster",m),T("failed")});let P=!1,_=0,C=0;const $=m=>{m.pointerType==="mouse"&&m.button!==0||(P=!0,_=m.clientX,C=0,g.setPointerCapture?.(m.pointerId),w(!0))},X=m=>{if(!P)return;const z=m.clientX-_;_=m.clientX,M.rotation.y+=z*se,C=C*.6+z*se*.4},V=()=>{P&&(P=!1,w(!1))};g.addEventListener("pointerdown",$),g.addEventListener("pointermove",X),g.addEventListener("pointerup",V),g.addEventListener("pointercancel",V);const Z=new o.Clock;let F=0,q=!1;const ce=()=>{const m=Math.min(Z.getDelta(),.05);if(!P){const z=ge*m;C+=(z-C)*.03,M.rotation.y+=C}h.render(k,x)},J=()=>{ce(),F=requestAnimationFrame(J)},de=()=>{q||(q=!0,Z.start(),F=requestAnimationFrame(J))},Q=()=>{q=!1,cancelAnimationFrame(F)};let ee=!0;const O=()=>{ee&&!document.hidden?de():Q()},te=new IntersectionObserver(([m])=>{ee=m.isIntersecting,O()},{threshold:0});te.observe(r);const ae=()=>O();document.addEventListener("visibilitychange",ae);const K=()=>{!r.clientWidth||!r.clientHeight||(x.aspect=r.clientWidth/r.clientHeight,x.updateProjectionMatrix(),h.setSize(r.clientWidth,r.clientHeight))};window.addEventListener("resize",K);const re=new ResizeObserver(K);re.observe(r),O(),d=()=>{Q(),te.disconnect(),re.disconnect(),document.removeEventListener("visibilitychange",ae),window.removeEventListener("resize",K),g.removeEventListener("pointerdown",$),g.removeEventListener("pointermove",X),g.removeEventListener("pointerup",V),g.removeEventListener("pointercancel",V),B.dispose(),A.dispose(),h.dispose(),g.parentNode===r&&r.removeChild(g)}})(),()=>{p=!0,d?.()}},[b,i,y,H]);const N=S!=="ready";return e.jsxs("div",{className:"cs-stage",style:{cursor:S==="ready"?f?"grabbing":"grab":"default"},children:[e.jsx("div",{ref:a,className:"cs-stage-mount","aria-hidden":S==="ready"?void 0:!0}),e.jsx("img",{src:s,alt:c,draggable:!1,className:"cs-stage-poster",style:{opacity:N?1:0},loading:"lazy"}),S==="ready"&&e.jsxs("span",{className:"cs-stage-hint",style:{color:y},children:[e.jsx("svg",{width:"14",height:"14",viewBox:"0 0 24 24",fill:"none",stroke:"currentColor",strokeWidth:"2",strokeLinecap:"round",strokeLinejoin:"round","aria-hidden":"true",children:e.jsx("path",{d:"M4 12h16M4 12l4-4M4 12l4 4M20 12l-4-4M20 12l-4 4"})}),"Drag to rotate"]}),e.jsx("style",{children:`
        .cs-stage {
          position: relative;
          width: 100%;
          height: 100%;
      ...
      `})]})}const be=6,ue=4;function fe({hero:i,heroAlt:s,backdrop:c}){const y=j.useRef(null),H=j.useRef(null);return j.useEffect(()=>{if(window.matchMedia("(prefers-reduced-motion: reduce)").matches)return;const a=y.current,b=H.current;if(!a||!b)return;let u=0,S=0,T=0,f=0,w=0;const N=()=>{T+=(u-T)*.08,f+=(S-f)*.08,b.style.transform=`perspective(1100px) rotateX(${f.toFixed(3)}deg) rotateY(${T.toFixed(3)}deg)`,Math.abs(u-T)>.02||Math.abs(S-f)>.02?w=requestAnimationFrame(N):w=0},r=()=>{w||(w=requestAnimationFrame(N))},p=o=>{if(o.pointerType!=="mouse")return;const v=a.getBoundingClientRect();u=((o.clientX-v.x)/v.width-.5)*2*be,S=-(((o.clientY-v.y)/v.height-.5)*2*ue),r()},d=()=>{u=0,S=0,r()};return a.addEventListener("pointermove",p),a.addEventListener("pointerleave",d),()=>{a.removeEventListener("pointermove",p),a.removeEventListener("pointerleave",d),cancelAnimationFrame(w)}},[]),e.jsxs("div",{ref:y,className:"csis",children:[e.jsx("img",{src:c,alt:"","aria-hidden":"true",className:"csis-backdrop",draggable:!1}),e.jsx("div",{className:"csis-scrim","aria-hidden":"true"}),e.jsx("div",{ref:H,className:"csis-tilt",children:e.jsx("img",{src:i,alt:s,className:"csis-hero",draggable:!1})}),e.jsx("style",{children:`
        .csis {
          position: relative;
          width: 100%;
          height: 100%;
